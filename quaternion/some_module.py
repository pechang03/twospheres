...
qx = quaternion.from_float_array([np.cos(30 * np.pi / 180 / 2), np.sin(30 * np.pi / 180 / 2), 0, 0])
qy = quaternion.from_float_array([np.cos(45 * np.pi / 180 / 2), 0, np.sin(45 * np.pi / 180 / 2), 0])
q_rot = qx * qy
...
for theta, phi in zip(theta.ravel(), phi.ravel()):
    ...
    rotated_point = q_rot * quaternion.quaternion(x, y, z, 0) * q_rot.conj()
